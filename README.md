...
3. 启动容器
4. 等待健康检查通过
5. 打开浏览器进入管理面板 `http://localhost:8000/admin`
6. 删除临时文件
...
`deploy_local.bat` 会创建一个临时覆写文件 `docker-compose.local.yml`，将 `PROXY_ADMIN_KEY` 环境变量设为空值，从而关闭管理面板的密码认证：

```
[创建 docker-compose.local.yml]
  → 写入 services.llm-failover.environment: PROXY_ADMIN_KEY=
[构建] docker compose -f docker-compose.yml -f docker-compose.local.yml build
[启动] docker compose -f docker-compose.yml -f docker-compose.local.yml up -d
[清理] 删除 docker-compose.local.yml
[等待健康检查] 每 3 秒探测一次 /health 端点
[打开浏览器] http://localhost:8000/admin
```

> 为什么不直接 `set PROXY_ADMIN_KEY=`？Windows cmd.exe 的 `set VAR=` 会**彻底移除**环境变量而不是置空，导致 `${PROXY_ADMIN_KEY:-admin123}` 回退到默认值 `admin123`。临时覆写文件确保空值正确传递到容器环境。
...
├── deploy_local.bat            # 本地部署脚本（无密码 + 自动打开管理面板）
├── requirements.txt            # 依赖：fastapi, uvicorn, httpx, pyyaml
├── docker-compose.yml          # Docker 部署（主用启动方式）
├── Dockerfile                  # Python 3.12-slim
├── design.md                   # 详细设计文档
├── dev-plan.md                 # 开发计划与功能需求
├── jiuwen-interruption-report.md  # Jiuwen 对话静默中断问题
├── throughput-optimization-plan.md  # 吞吐优化方案
└── app/
